...
void PolygonShape::set(const std::vector<glm::vec2>& vertices) {
...
void PolygonShape::setAsBox(glm::vec2 size) {
  size = world->pixelsToWorld(size);
  polygon.SetAsBox(size.x / 2, size.y / 2);
}

void CircleShape::set(const glm::vec2& position, float radius) {
